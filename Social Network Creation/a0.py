...
def friend_overlap(users):
    ...
    for i in range(len(users)):
        for j in range(i+1,len(users)):
            if users[i]!=users[j]:
                overlap_count = set(users[i]['friends']).intersection(users[j]['friends'])
                shared_accounts.append((users[i]['screen_name'], users[j]['screen_name'], len(overlap_count)))
 
    return sorted(shared_accounts, key=lambda accounts: (-accounts[2],accounts[0],accounts[1]))
    pass
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    """
    ###TODO
    donald_request = twitter.request('friends/ids', {'screen_name': "realDonaldTrump", 'count':5000})
    hillary_request = twitter.request('friends/ids', {'screen_name': "HillaryClinton", 'count':5000})
    
    common_user = set(donald_request).intersection(hillary_request)
    twitter_user = twitter.request('users/lookup', {'user_id': common_user})
...
def create_graph(users, friend_counts):
    ...
    for f in friend_counts:
        followers_count= friend_counts[f]
